...
<head>
  ...
  ================================================== -->
  <meta charset="utf-8">
  <title>Evolv Domain Companies - DASHBOARD</title>
		...
		<div class="row">
			...
			''	dim ShowTypeSTR, SRCWordSTR, GTcountrySTR
				...
				Dim Start1, StartBack
					...
					<%''=ucase(trim(ShowTypeSTR))%>COMPANIES IN <%=ucase(trim(GTcountrySTR))%></h2>		
			...
			<div class="col-12 mx-auto"  >
				...
				<%end if%>
							<p class="Alp_Display  text-center  "> 
					<%'if trim(chkOpt)="Company" then%>					
					<a class="<%=SelectedActive(SRCWordSTR,"[0-9]") %>" href="Show_Domain_Companies.asp?GTcountry=<%=trim(REGTcountrySTR)%>&ShowType=<%=trim(REShowTypeSTR)%>&CHKType=<%=trim(RECHKTypeSTR)%>&StupType=<%=trim(request("StupType"))%>&SRCWord=<%=server.urlencode(EnDeCrypt("[0-9]",CHkGiaPSWD))%>">0-9</a>&nbsp;
					<%'end if%>
								...
								<div class="col-12 card-desing-1 mx-auto p-2">
									...
									<div class="d-flex justify-content-start align-items-center flex-column">
										...
										<div class="right-block">
											<div class="title pl-2 text-center">
