...
<%
	...
	RecChkSQL = "SELECT * FROM Evolv_Membership WHERE Email ='"&trim(request("vEmail"))&"' and Member_Type='"&membershipType&"' and Invite_Code = '"&trim(request("vInvite_code"))&"'" 
	...
	If RecChkRS.EOF Then		
		 InsertSQL = "INSERT INTO Evolv_Membership ([First_Name],[Last_Name],[Email],[Mobile],[LinkedInLink],[Location],[Member_Type],[SubmittedDate],[Status],[Designation],[Company_Name],[Invite_Code],[Participant_Bio]) VALUES ('"&trim(request("vFirst_Name"))&"','"&trim(request("vLast_Name"))&"','"&trim(request("vEmail"))&"','"&trim(request("vPhone"))&"','"&trim(request("vLinkedin"))&"','"&trim(request("vlocation"))&"','"&membershipType&"',getdate(),'Y','"&GTDesignation&"','"&GTCompany&"','"&trim(request("vInvite_code"))&"','"&trim(ParticipantBio)&"')" 
