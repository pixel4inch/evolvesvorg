...
<head>
  ...
  ================================================== -->
  <meta charset="utf-8">
  <title>Companies Details - Evolve SV</title>
  ...
  <section class="section pt-3">
    <div class="container">
	 ...
	 <%
			...
			Dim CompNameDisp
			CompNameDisp = trim(request("cmpName"))
			...
			If trim(request("CmpCheck"))="Investor" Then
				SelectQry="select Organization_Person_Name as organization_name,company_image as Company_Image_url from CB_Investors  where [Organization_Person_Name] like ('%" & trim(request("cmpName")) & "%')"
			End If
			
			'''response.write SelectQry
			...
			If trim(RSWhere("organization_name"))<>"" Then
				CompNameDisp = trim(RSWhere("organization_name"))
			End If
    ...
    <div class="mt-0   " id="domain-experts" data-parent="#collapseblock" >  
		...
		<h1 class="font-display2 text-center main-font-color col-lg-11 col-md-12 mx-auto" style="text-transform: uppercase;"><%=ucase(trim(request("CmpType")))%></h1>
...
">
		...
		<h1 class="font-display1 text-center main-font-color col-lg-11 col-md-12 mx-auto" style="text-transform: uppercase;"><%=trim(CompNameDisp)%></h1>
	</div>
